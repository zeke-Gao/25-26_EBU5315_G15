...
const i18n = {
  en: {
    ...
    hero_title: "Master Circle Geometry with interactive games and quiz",
    hero_sub:
    ...
    jump_title: "Practice and Play",
    jump_sub: "Choose an interactive module to continue learning.",
    jump_game_title: "Goose Parametric Game",
    jump_game_text:
      "Tune geometric parameters in sandbox or challenge mode and compare overlap in real time.",
    jump_game_cta: "Enter Game",
    jump_quiz_title: "Circle Quiz",
    jump_quiz_text:
      "Answer mixed concept and diagram questions with instant feedback and score tracking.",
    jump_quiz_cta: "Start Quiz",
    ads_label: "Sponsored",
  ...
  zh: {
    ...
    jump_title: "继续互动学习",
    jump_sub: "选择你想进入的模块：参数游戏或测验练习。",
    jump_game_title: "大鹅参数化游戏",
    jump_game_text: "通过拖动参数实时观察几何模型变化，在挑战模式下提升图形重合度。",
    jump_game_cta: "进入 Game",
    jump_quiz_title: "圆几何测验",
    jump_quiz_text: "完成概念题与图形题混合训练，获得即时反馈并追踪你的得分表现。",
    jump_quiz_cta: "进入 Quiz",
    ads_label: "赞助",
...
function toggleChat(open) {
  ...
  if (!open && geobotForm && geobotForm.style.display === "grid" && formHasContent()) {
...
let lastTheoremIndex = { en: -1, zh: -1 };

function getRandomTheorem(lang) {
  const pool = circleTheorems[lang] || circleTheorems.en;
  if (pool.length === 0) return null;
  if (pool.length === 1) return pool[0];

  let index = Math.floor(Math.random() * pool.length);
  while (index === lastTheoremIndex[lang]) {
    index = Math.floor(Math.random() * pool.length);
  }
  lastTheoremIndex[lang] = index;
  return pool[index];
}
...
if (geobotQuick) {
  geobotQuick.addEventListener("click", (event) => {
    ...
    if (action === "learn") {
      const theorem = getRandomTheorem(currentLang);
      if (theorem) {
        addMessage(theorem);
      }
    }
    ...
    if (action === "play") {
      const quizUrl = currentLang === "zh" ? "../Quiz/test-zh.html" : "../Quiz/test.html";
